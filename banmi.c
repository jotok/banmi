...
sort_data_by_missingness_pattern(banmi_model_t *model) {
    ...
    banmi_order_blocks_int(model->disc->data, model->mis_pat, 
                           model->n_rows, model->disc->tda);
    banmi_order_blocks_int(model->disc_imp->data, model->mis_pat, 
                           model->n_rows, model->disc_imp->tda);
    banmi_order_blocks(model->cont->data, model->mis_pat, 
                       model->n_rows, model->cont->tda);
    banmi_order_blocks(model->cont_imp->data, model->mis_pat, 
                       model->n_rows, model->cont_imp->tda);
...
init_missing_values(gsl_rng *rng, banmi_model_t *model) {
    ...
    for (i = model->n_complete; i < model->n_rows; i++) {
        ...
        if (this_pattern & model->mask_missing_disc_data) {
            ...
            flat_ix = banmi_sample(rng, margtab->size, margtab->dat);
...
init_latent_variables(gsl_rng *rng, banmi_model_t *model) {
    ...
    for (i = 0; i < model->n_rows; i++) {
        ...
        flat_ix = banmi_sample(rng, model->crosstab->size, model->crosstab->dat);
...
draw_new_latent_variables(gsl_rng *rng, banmi_model_t *model) {
    ...
    for (i = 0; i < model->n_rows; i++) {
        ...
        choice = banmi_sample(rng, model->n_rows, weight);
